package org.jasig.cas.nhnent.web.flow;
 ...
 */
public class AuthenticationExceptionHandlerForNhn {
    ...
    public void setMemcachedIO(MemcachedIO memcachedIO) {
            ...
            for (final Class<? extends Exception> kind : this.errors) {
                for (final Class<? extends Exception> handlerError : e.getHandlerErrors().values()) {
                    if (handlerError != null && handlerError.equals(kind)) {
                        ...
                        if(handlerErrorName.contentEquals("FailedLoginException")) {
                            Integer failureCount = (Integer) memcachedIO.get(getCacheKey());
                            if (failureCount == null) {
                                failureCount = 0;
                            }
                            messageContext.addMessage(builder.error().code(messageCode).arg(failureCount + 1).build());
                        } else {
